...
class HTTPServer(object):
    ...
    def _request_end(self, client_socket):
        ...
        while True:
            # 完整读取请求数据
            recv_data_s = client_socket.recv(1024)
            if recv_data_s == b'':
                request_line = '0'
            ...
            try:
                ...
                print('》》》'+ str(e))
                print(recv_data_s)
                recv_data_s = recv_data_s.decode('gb2312')


            if "\r\n\r\n" in recv_data_s:
                # 有空行
                if request_header == '':
                    space_line_index = recv_data_s.index("\r\n\r\n")
                    request_header = recv_data_s[0: space_line_index]
                    content_entity = recv_data_s[space_line_index + 4:]
                    ...
                    # 第一次接收到，空行 说明请求头结束
                    if "content_length" in request_headers.keys():
                        # 查看content_length是否在请求头,若在，需要获取其值
                        content_length = request_headers['content_length']
